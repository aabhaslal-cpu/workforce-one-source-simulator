...
export async function runConnectorTestKit(): Promise<ConnectorKitReport> {
  const simulator = await SourceSimulator.create({ seed: "connector-kit-seed", baseUrl: "http://sim.test" });
  const connectionId = "conn-product-vp";
  const steps: ConnectorKitStep[] = [];

  const initial = SourceFeedBatchV1Schema.parse(await simulator.feed(connectionId, undefined, 25));
  steps.push({
    name: "initial_sync",
  ...
  await simulator.triggerScenarioInstanceEvent(instanceId, manualEvent.id);
  const triggered = SourceFeedBatchV1Schema.parse(await simulator.feed(connectionId, savedCursor, 25));
  steps.push({
  ...
  const retry = SourceFeedBatchV1Schema.parse(await simulator.feed(connectionId, savedCursor, 25));
  steps.push({
  ...
  await simulator.advanceScenarioInstance(instanceId, { hours: 48 });
  const afterUpdate = SourceFeedBatchV1Schema.parse(await simulator.feed(connectionId, triggered.nextCursor, 25));
  steps.push({
  ...
  await simulator.resetScenarioInstance(instanceId);
  let staleCursorRejected = false;
  try {
    await simulator.feed(connectionId, beforeResetCursor, 25);
  } catch {
    staleCursorRejected = true;
  }
  const newCursorPage = SourceFeedBatchV1Schema.parse(await simulator.feed(connectionId, undefined, 25));
  steps.push({
  ...
  const managerRecords = new Set(
    SourceFeedBatchV1Schema.parse(await simulator.feed("conn-product-manager", undefined, 100)).records.map((record) => record.sourceId),
  );
  const icRecords = SourceFeedBatchV1Schema.parse(await simulator.feed("conn-product-ic", undefined, 100)).records.map((record) => record.sourceId);
  steps.push({
  ...
  await simulator.regenerateOrganization({ seed: "connector-kit-rotation" });
