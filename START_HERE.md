...
7. Read `docs/OPERATIONS.md`, `docs/STORAGE.md`, `docs/FAILURE_MODES.md`, and `docs/TESTING.md` before changing deployment, storage, health, metrics, or failure simulation.
8. Run `pnpm install --frozen-lockfile` and `pnpm run verify` before declaring the PR ready.
9. Check GitHub Actions because CI runs Postgres parity tests.
...
8. OpenAPI, JSON Schema, migrations, examples, and docs match runtime behavior.
9. CI is green with Postgres parity tests.
